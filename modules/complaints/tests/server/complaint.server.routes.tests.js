...
describe('Complaint CRUD tests', function () {
          ...
          .end(function (complaintSaveErr, complaintSaveRes) {
            ...
            agent.get('/api/complaints')
              .end(function (complaintsGetErr, complaintsGetRes) {
                ...
                // Set assertions
                (complaints[0].user._id).should.equal(userId);
                (complaints[0].name).should.match('Complaint name');
  ...
  it('should not be able to save an Complaint if not logged in', function (done) {
    agent.post('/api/complaints')
      .send(complaint)
      .expect(403)
      .end(function (complaintSaveErr, complaintSaveRes) {
